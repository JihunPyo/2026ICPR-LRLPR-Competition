...
class TrackSequenceDataset(Dataset):
    """Track-level dataset for 5-frame LR and variable-count HR pairs."""
    ...
    @classmethod
    def _collect_hr_frames(cls, track_dir: Path) -> List[Path]:
        """Collect HR frames with wildcard pattern (e.g., hr-003, hr-157)."""
        hr_candidates = []
        for path in track_dir.iterdir():
            if not path.is_file():
                continue
            if path.suffix.lower() not in cls.VALID_IMAGE_EXTS:
                continue
            if not path.name.lower().startswith("hr-"):
                continue
            hr_candidates.append(path)

        def _sort_key(path: Path):
            stem = path.stem  # hr-003
            idx = None
            if "-" in stem:
                tail = stem.split("-", 1)[1]
                if tail.isdigit():
                    idx = int(tail)
            return (idx is None, idx if idx is not None else 10**9, path.name.lower())

        hr_candidates.sort(key=_sort_key)
        ...
        for scenario_dir in sorted(train_root.glob("Scenario-*")):
            ...
            for layout_dir in sorted(scenario_dir.glob("*")):
                ...
                for track_dir in sorted(layout_dir.glob("track_*")):
                    lr_paths = []
                    valid_lr_frames = True
                    for i in range(1, 6):
                        lr_p = self._find_frame(track_dir, "lr", i)
                        if lr_p is None:
                            valid_lr_frames = False
                            break
                        lr_paths.append(lr_p)

                    hr_paths = self._collect_hr_frames(track_dir)

                    anno_path = track_dir / "annotations.json"
                    if not anno_path.exists():
                        continue
                    if not valid_lr_frames:
                        continue
                    if not hr_paths:
                        continue
