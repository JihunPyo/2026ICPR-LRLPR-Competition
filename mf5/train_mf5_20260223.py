...
import models
from losses.lossPack_mf5 import LossPackMF5
...
def validate_mf5_train_config(config: dict):
    ...
    losspack_cfg = loss_cfg.get("losspack_mf5", {})
    ...
    lcofl_weight_default = 1.0 if loss_mode in {"lcofl_only", "losspack_mf5"} else 0.0
    ...
    losspack_ce_impl = str(losspack_cfg.get("ce_impl", "onehot")).lower()

    if loss_mode in {"lcofl_only", "losspack_mf5"}:
        if not model_ckpt:
            raise ValueError(
                "`model.checkpoint` must be set in `loss.mode=lcofl_only|losspack_mf5`."
            )
        if not ocr_cfg.get("ocr_ckpt"):
            raise ValueError(
                "`ocr.ocr_ckpt` must be set in `loss.mode=lcofl_only|losspack_mf5`."
            )
        if lcofl_weight <= 0.0:
            raise ValueError("`loss.lcofl_weight` must be > 0 in `loss.mode=lcofl_only|losspack_mf5`.")
        ...
        if update_mode != "avg5_once":
            raise ValueError(
                "`train.update_mode` must be `avg5_once` in `loss.mode=lcofl_only|losspack_mf5`."
            )
        if loss_mode == "losspack_mf5":
            if losspack_ce_impl not in {"onehot", "logits"}:
                raise ValueError(
                    f"Unsupported loss.losspack_mf5.ce_impl: {losspack_ce_impl}. Supported: onehot, logits"
                )
            forbidden_keras_keys = {"load", "loss_specs", "keras", "tensorflow", "tf"}
            active_forbidden = sorted(forbidden_keras_keys.intersection(losspack_cfg.keys()))
            if active_forbidden:
                raise ValueError(
                    "loss.mode=losspack_mf5 does not support TensorFlow/Keras options. "
                    f"Remove keys: {active_forbidden}"
                )
        need_ocr = True
    else:
    ...
    return {
        ...
        "losspack_ce_impl": losspack_ce_impl if loss_mode == "losspack_mf5" else None,
        "lcofl_terms": {
...
def train_one_epoch(
    ...
    if loss_mode in {"lcofl_only", "losspack_mf5"} and lcofl_criterion is None:
        raise ValueError("`lcofl_criterion` must be provided in `loss.mode=lcofl_only|losspack_mf5`.")
...
def main():
    ...
    ocr_model_args = None
    ocr_checkpoint = None
    need_ocr = True if loss_mode in {"lcofl_only", "losspack_mf5"} else ((lcofl_weight > 0.0) or adv_enabled)
    need_ocr = need_ocr or val_ocr_enabled
    if need_ocr:
        ...
        elif loss_mode == "losspack_mf5":
            lcofl_criterion = LossPackMF5(
                alphabet=ocr_model_args.get("alphabet", "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ"),
                k=ocr_k,
                ce_weight=lcofl_ce_weight,
                ssim_weight=lcofl_ssim_weight,
    ...
    if is_main_process(distributed):
        ...
        if loss_mode == "losspack_mf5":
            print(f"losspack ce_impl: {losspack_ce_impl}")
            if losspack_ce_impl == "onehot" and lcofl_ce_weight > 0:
                print(
                    "[WARN] loss.mode=losspack_mf5 with ce_impl=onehot keeps original-style CE, "
                    "so SR gradient flow from CE can be limited."
                )
